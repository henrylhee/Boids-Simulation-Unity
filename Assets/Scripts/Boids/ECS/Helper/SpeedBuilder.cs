namespace Boids
{
    ...
    public struct SpeedBuilder
    {
        private EntityQuery query;
        private ComponentTypeHandle<CSpeed> transformHandle;


        public SpeedBuilder(ref SystemState state, EntityQuery query)
        {
            this.query = query;

            this.transformHandle = state.GetComponentTypeHandle<CSpeed>(true);
        }

        public JobHandle Gather(ref SystemState state, JobHandle dependency, ref NativeArray<CSpeed> positions)
        {
        ...
        {
            [ReadOnly]
            public ComponentTypeHandle<CSpeed> TransformHandle;

            [NativeDisableParallelForRestriction]
            public NativeArray<CSpeed> Positions;
            ...
            {
                var ptr = (float*)this.Positions.GetUnsafePtr();
                var dst = ptr + this.FirstEntityIndices[unfilteredChunkIndex];

                var size = UnsafeUtility.SizeOf<float>();
                var positions = chunk.GetNativeArray(ref this.TransformHandle).Slice().SliceWithStride<float>(0);
